/*
 *  serialsim - A python interface to the linux serialsim module.
...
static void err_handle(char *name, int rv)
{
    if (!rv)
	return;
    PyErr_Format(PyExc_Exception, "%s: %s", name, strerror(rv));
}
 ...
 * module does.
 */
void get_remote_termios(int fd, void *termios) {
    int rv;
...
void set_remote_modem_ctl(int fd, unsigned int val) {
    int rv;
...
unsigned int get_remote_modem_ctl(int fd) {
    int rv;
    unsigned int val;
...
void set_remote_serial_err(int fd, unsigned int val) {
    int rv;
...
unsigned int get_remote_serial_err(int fd) {
    int rv;
    unsigned int val;
...
void set_remote_null_modem(int fd, bool val) {
    int rv;
...
bool get_remote_null_modem(int fd) {
    int rv, val;
 ...
 */
%constant int SERIALSIM_TIOCM_CAR = TIOCM_CAR;
%constant int SERIALSIM_TIOCM_CTS = TIOCM_CTS;
%constant int SERIALSIM_TIOCM_DSR = TIOCM_DSR;
%constant int SERIALSIM_TIOCM_RNG = TIOCM_RNG;
%constant int SERIALSIM_TIOCM_DTR = TIOCM_DTR;
%constant int SERIALSIM_TIOCM_RTS = TIOCM_RTS;

/* For remote errors.  These are the kernel numbers. */
%constant int SERIALSIM_TTY_BREAK = 1 << 1;
%constant int SERIALSIM_TTY_FRAME = 1 << 2;
%constant int SERIALSIM_TTY_PARITY = 1 << 3;
%constant int SERIALSIM_TTY_OVERRUN = 1 << 4;
